...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'libcash' : 'libcash';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/libcash-build/';
  ...
  task['test:karma'] = shell.task([
    karmaPath + '  start karma.conf.js --single-run '
  ]);
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external libcash -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:libcash -o libcash.js';
    }
