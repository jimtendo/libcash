...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new libcash.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new libcash.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = libcash.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new libcash.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function libcashTest() {
      block1 = libcash.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('libcash', libcashTest, {maxTime: maxTime});
